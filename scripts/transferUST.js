...
async function main() {
  ...
  const ust = NewToken.attach(addresses.token3);
  
  // Convert amount to the token's smallest unit (wei)
  const amountInWei = ethers.parseEther(amount);
  
  // Transfer tokens
  console.log(`Transferring ${amount} UST tokens to ${recipientAddress}...`);
  const tx = await ust.transfer(recipientAddress, amountInWei);
